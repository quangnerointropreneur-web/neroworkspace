    ...
    <div className="animate-fadeIn" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              {isExpanded && (
                <div style={{ padding: "16px 20px", background: "var(--bg-secondary)", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
                    ...
                    <div>
                      <label style={{ fontSize: 11, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>Chốt ngày đi làm</label>
                      <input
                        type="number"
                        min={0}
                        max={31}
                        value={att?.workDays ?? 0}
                        onChange={(e) => updateAttendance(u.id, "workDays", +e.target.value)}
                        style={{ ...input, width: 80 }}
                      />
                    </div>
                    <div>
                      <label style={{ fontSize: 11, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>Nghỉ phép</label>
                      <input
                        type="number"
                        min={0}
                        max={31}
                        value={att?.leaveDays ?? 0}
                        onChange={(e) => updateAttendance(u.id, "leaveDays", +e.target.value)}
                        style={{ ...input, width: 80 }}
                      />
                    </div>
                    <div>
                      <label style={{ fontSize: 11, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>Tổng chuẩn</label>
                      <input
                        type="number"
                        value={att?.totalWorkingDays ?? 26}
                        readOnly
                        style={{ ...input, width: 80, opacity: 0.5 }}
                      />
                    </div>
                    <div>
                      <label style={{ fontSize: 11, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>Tỷ lệ</label>
                      <div style={{ padding: "8px 12px", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 8, fontSize: 13, fontWeight: 700, color: "#10b981" }}>
                        {att ? Math.round((att.workDays / (att.totalWorkingDays || 26)) * 100) : 0}%
                      </div>
                    </div>
                    ...
                    <div style={{ marginLeft: "auto", background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: 10, padding: "10px 16px" }}>
                      ...
                      <div style={{ marginTop: 6, paddingTop: 6, borderTop: "1px solid rgba(255,255,255,0.07)", fontSize: 14, fontWeight: 700, color: "#60a5fa" }}>
                        Thực lĩnh: {formatVND(income)}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)", padding: 20 }} onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="animate-scaleIn" style={{ width: "100%", maxWidth: 520, background: "var(--bg-card)", borderRadius: 20, border: "1px solid var(--border)", boxShadow: "0 32px 80px rgba(0,0,0,0.5)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "var(--text-primary)" }}>{editingUser ? "Sửa nhân viên" : "Thêm nhân viên"}</h3>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  ...
                  <input value={fFullName} onChange={(e) => setFFullName(e.target.value)} placeholder="Nguyễn Văn A" style={input} />
                </div>
                <div>
                  <label style={lbl}>Phòng ban</label>
                  <input value={fDept} onChange={(e) => setFDept(e.target.value)} placeholder="Marketing" style={input} />
                </div>
                <div>
                  <label style={lbl}>Username *</label>
                  <input value={fUsername} onChange={(e) => setFUsername(e.target.value)} placeholder="nguyen.van.a" style={input} />
                </div>
                <div>
                  <label style={lbl}>Mật khẩu</label>
                  <input type="password" value={fPassword} onChange={(e) => setFPassword(e.target.value)} placeholder={editingUser ? "Giữ nguyên nếu để trống..." : "pass123"} style={input} />
                </div>
                <div>
                  <label style={lbl}>Vai trò</label>
                  <select value={fRole} onChange={(e) => setFRole(e.target.value as any)} style={{ ...input, cursor: "pointer" }}>
                    <option value="employee">Nhân viên</option>
                    <option value="admin">Admin</option>
                  </select>
                </div>
                <div>
                  <label style={lbl}>Lương cứng (VNĐ)</label>
                  <input type="number" value={fBaseSalary} onChange={(e) => setFBaseSalary(+e.target.value)} style={input} />
                </div>
                <div>
                  <label style={lbl}>Thưởng (VNĐ)</label>
                  <input type="number" value={fBonus} onChange={(e) => setFBonus(+e.target.value)} style={input} />
                </div>
                <div>
                  <label style={lbl}>Phạt (VNĐ)</label>
                  <input type="number" value={fPenalty} onChange={(e) => setFPenalty(+e.target.value)} style={input} />
                </div>
                <div>
                  <label style={lbl}>Giờ vào ca</label>
                  <input type="time" value={fShiftStart} onChange={(e) => setFShiftStart(e.target.value)} style={input} />
                </div>
                <div>
                  <label style={lbl}>Giờ kết thúc ca</label>
                  <input type="time" value={fShiftEnd} onChange={(e) => setFShiftEnd(e.target.value)} style={input} />
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                ...
                <button onClick={handleSave} style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 22px", borderRadius: 10, background: "linear-gradient(135deg, #3b82f6, #8b5cf6)", border: "none", color: "white", cursor: "pointer", fontSize: 14, fontWeight: 600 }}>
