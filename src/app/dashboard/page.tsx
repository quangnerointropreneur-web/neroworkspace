    ...
    <div className="animate-fadeIn" style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Brand quick stats (admin only) */}
          {isAdmin && (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
              <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: 6 }}>
                  <Briefcase size={15} /> Brands
                </h3>
                <Link href="/dashboard/brands" style={{ color: "var(--accent-blue)", fontSize: 12, textDecoration: "none" }}>Chi tiết →</Link>
              </div>
              {state.brands.map((brand) => {
                const brandTasks = state.tasks.filter((t) => t.brandId === brand.id);
                const doneTasks = brandTasks.filter((t) => t.status === "done").length;
                const progress = brandTasks.length > 0 ? Math.round((doneTasks / brandTasks.length) * 100) : 0;
                return (
                  <div key={brand.id} style={{ padding: "12px 18px", borderBottom: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 9, height: 9, borderRadius: "50%", background: brand.color }} />
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{brand.name}</span>
                      </div>
                      <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{doneTasks}/{brandTasks.length}</span>
                    </div>
                    <div style={{ height: 5, background: "var(--bg-secondary)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${progress}%`, background: brand.color, borderRadius: 3, transition: "width 0.5s" }} />
                    </div>
                  </div>
                );
              })}
